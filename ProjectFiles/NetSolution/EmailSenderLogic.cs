...
public class EmailSenderLogic : BaseNetLogic
{
    ...
    [ExportMethod]
    public void SendEmail(string mailToAddress, string ccRecipients,string mailSubject, string mailBody, string attachFilePath)
    {
        ...
        var message = CreateEmailMessage(fromAddress, toAddress, mailBody, mailSubject, attachFilePath, ccRecipients);
        TrySendEmail(message);
    }

    private MailMessageWithRetries CreateEmailMessage(MailAddress fromAddress, MailAddress toAddress, string mailBody, string mailSubject, string attachFilePath, string ccRecipients)
    {
        ...
        //usea Attachment ResourceUri from Script
        //var attachment = GetVariableValue("Attachment").Value;

        // Add CC recipients if provided
        if (!string.IsNullOrEmpty(ccRecipients))
        {
            var ccEmails = ccRecipients.Split(new char[] { ',', ';' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        //Use Attachment String from method
        var attachment = attachFilePath;
