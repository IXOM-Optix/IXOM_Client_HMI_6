...
public class Replay_ALZ : BaseNetLogic
{
    ...
    public override void Start()
  {
    ...
    private void ReplayTime_VariableChange(object sender, VariableChangeEventArgs e)
      {
        ...
        _Value = LogicObject.GetVariable("Replay/Value");
    ...
    //Value
     ...
     string queryString = $"SELECT Value FROM "+Device_Name+" WHERE LocalTimestamp <= '" + sqlFormattedDate + "'ORDER BY LocalTimestamp DESC LIMIT 2";
        myStore.Query(queryString, out _, out resultSet);
     //Store Valuefrom Query
      _Value.Value = (dynamic)resultSet[0, 0];
